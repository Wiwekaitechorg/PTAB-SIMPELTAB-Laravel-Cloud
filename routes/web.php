...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'namespace' => 'Admin', 'middleware' => ['auth']], function () {
    ...
    // keluhan pelanggan
    Route::get('customers/viewReport', 'CustomersController@viewReport')->name('customers.viewReport');
    Route::post('customers/reportExcel', 'CustomersController@reportExcel')->name('customers.reportExcel');
    ...
    Route::delete('complains/destroy', 'ComplainsController@massDestroy')->name('complains.massDestroy');
    Route::get('complains-ticket-create', 'ComplainsController@ticketCreate')->name('complains.ticketCreate');
    Route::post('complains-ticket-store', 'ComplainsController@ticketStore')->name('complains.ticketStore');
    Route::get('reports/reportscomplain', 'ReportsController@reportComplain')->name('report.complain');
    Route::post('reports/reportscomplain/proses', 'ReportsController@reportComplainProses')->name('report.complainproses');
    //pbk
    Route::get('complain/pbk', 'ComplainsController@complainPbk')->name('complain.complainsPbk');
    Route::get('complain/opp', 'ComplainsController@complainOpp')->name('complain.complainsOpp');
    Route::get('complain/opp/create/', 'ComplainsController@complainOppCreate')->name('complain.complainOppCreate');
    Route::post('complain/opp/store/', 'ComplainsController@complainOppStore')->name('complain.complainOppStore');
    Route::delete('complain/opp/delete/{operator}/{opp}', 'ComplainsController@complainOppDestroy')->name('complain.complainOppDestroy');

    //check
    Route::get('checks/printservice', 'ChecksController@printservice')->name('checks.printservice');
    Route::get('checks/printspk', 'ChecksController@printspk')->name('checks.printspk');
    Route::get('checks/printreport', 'ChecksController@printreport')->name('checks.printreport');
    // end surya buat
    Route::resource('checks', 'ChecksController', ['only' => ['index', 'store', 'edit', 'update', 'destroy']]);
    Route::get('checks/create/{ticket_id}', 'ChecksController@create')->name('checks.create');
    Route::post('checks/staff', 'ChecksController@staff')->name('checks.staff');
    Route::get('checks/list/{check}', 'ChecksController@list')->name('checks.list');
    Route::delete('checks/destroy', 'ChecksController@massDestroy')->name('checks.massDestroy');
    Route::get('checks/staff/{check}', 'ChecksController@checkStaff')->name('checks.checkStaff');
    Route::get('checks/staff/create/{check}', 'ChecksController@checkStaffCreate')->name('checks.checkStaffCreate');
    Route::post('checks/staff/store/', 'ChecksController@checkStaffStore')->name('checks.checkStaffStore');
    Route::get('checks/staff/{check}/edit', 'ChecksController@checkStaffEdit')->name('checks.checkStaffEdit');
    Route::put('checks/staff/update', 'ChecksController@checkStaffUpdate')->name('checks.checkStaffUpdate');
    //   30-01-2024 start
    Route::get('checks/addiyionaldone/{check}/edit', 'ChecksController@additionalDone')->name('checks.additionalDone');
    Route::put('checks/addiyionaldone/update', 'ChecksController@storeAdditionalDone')->name('checks.storeAdditionalDone');
    Route::put('checks/staff/update', 'ChecksController@checkStaffUpdate')->name('checks.checkStaffUpdate');
    ...
    Route::get('ctmupdatemanual', 'CtmRequestController@updateManual');
    ...
    Route::get('test-file-exit', 'TestController@existFile');
    Route::get('test-absen', 'TestController@testabsen');

    Route::get('test-gdrive', 'TestController@gcsbucketIndex');
    Route::put('test-gdrive-store', 'TestController@gcsbucketStore')->name('test.gdriveStore');
    Route::get('gdrive-image', 'TestController@getGcsImage')->name('test.gdriveImage');



    Route::get('test-google-drive', function() {
        $filename = '5141_2023_10.jpg';
        $filePath = "/home/simpletabadmin-ssh/htdocs/simpletabadmin.ptab-vps-storage.com/gambar-test/202407/5141_2023_10.jpg";
        ...
        //get file url on store
        $disk = Storage::disk('google');
        $url = $disk->url($filename);        

        //delete file
        //Storage::disk('google')->delete('1ZR08j2C0-Ll-Lx0gd0W2B35dIbtlPJoT');

        //list
        $data = Storage::disk('google')->allFiles();

        //get meta data
        $metadata = Storage::disk('google')->getMetadata('1sGen4hyeZrOitNxbjAWIQ4tDHOou-enh');        

        //get list where
        //recursive true inclue sub folder
        ...
        //get url using id
        $urlid = Storage::disk('google')->url($fileid);

        return $urlid;
    });

    Route::resource('absenceadj', 'AbsenceAdjController');
    Route::delete('absenceadj/destroy', 'AbsenceAdjController@massDestroy')->name('absenceadj.massDestroy');
    Route::get('absence-recap', 'AbsenceAdjController@absenceRecap')->name('absenceRecap');
    Route::get('absence-recap-test', 'AbsenceAdjController@absenceRecapTest')->name('absenceRecapTest');
    ...
    // 26-04-2024
    Route::get('proposal_ka', 'ProposalKaController@index')->name('proposalKa.index');
    Route::get('proposal_ka/{id}/edit', 'ProposalKaController@edit')->name('proposalKa.edit');
    Route::put('proposal_ka/{id}/update', 'ProposalKaController@update')->name('proposalKa.update');
    ...
    // 07-03-2024 start
    Route::post('absence/absenceDelete', 'AbsenceController@absenceDelete')->name('absence.absenceDelete');
    Route::get('absence/viewDelete', 'AbsenceController@viewDelete')->name('absence.viewDelete');
    // 07-03-2024 end
    ...
    //opp customer operator
    Route::get('opp', 'CustomersController@indexOpp')->name('opp.index');
    Route::get('opp/show', 'CustomersController@showOpp')->name('opp.show');
    Route::get('opp/edit', 'CustomersController@editOpp')->name('opp.edit');
    Route::put('opp/update', 'CustomersController@updateOpp')->name('opp.update');
    //opp report
    Route::get('opp/viewReport', 'CustomersController@oppviewReport')->name('opp.viewReport');
    Route::post('opp/reportExcel', 'CustomersController@oppreportExcel')->name('opp.reportExcel');

    //dispense
    Route::post('dispense/{id}/approve', 'DispenseController@approve')->name('dispense.approve');
    Route::post('dispense/{id}/reject', 'DispenseController@reject')->name('dispense.reject');
