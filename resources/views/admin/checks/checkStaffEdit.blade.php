...
<div class="card">
    ...
    <div class="card-body">
        <form check="{{route('admin.checks.checkStaffUpdate')}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name='check_id' value='{{$check->id}}'>
            <input type="hidden" name='ticket_id' value='{{$check->ticket_id}}'>
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                <label for="code">{{ trans('global.check.fields.code') }}*</label>
                <input type="text" disabled id="code" name="code" class="form-control" value="{{$check->ticket->code}}" >
                @if($errors->has('code'))
            ...
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <input type="text" disabled id="description" name="description" class="form-control" value="{{$check->description}}" >
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                <label for="status">{{ trans('global.check_staff.fields.status') }}*</label>
                <select id="status" name="status" class="form-control" value="{{ old('status', isset($check) ? $check->status : '') }}">
                    <option value="">--Pilih status--</option>
                    <option value="pending" {{$check->status == 'pending' ? 'selected' :''}} >Pending</option>
                    <option value="active" {{$check->status == 'active' ? 'selected' :''}} >Active</option>
                    <option value="close" {{$check->status == 'close' ? 'selected' :''}} >Close</option>                    
                </select>
            ...
            <div class="form-group {{ $errors->has('todo') ? 'has-error' : '' }}">
                <label for="todo">{{ trans('global.check_staff.fields.todo') }}*</label>
                <select id="todo" name="todo" class="form-control" value="{{ old('todo', isset($check) ? $check->todo : '') }}">
                    <option value="">--Pilih Pekerja--</option>
                    <option value="Internal" {{$check->todo == 'Internal' ? 'selected' :''}} >Internal</option>
                    <option value="Pihak ke-3" {{$check->todo == 'Pihak ke-3' ? 'selected' :''}} >Pihak ke-3</option>          
                </select>
                @if($errors->has('todo'))
                    <em class="invalid-feedback">
                        {{ $errors->first('todo') }}
            ...
            <div class="form-group {{ $errors->has('memo') ? 'has-error' : '' }}">
                <label for="memo">{{ trans('global.check.fields.memo') }}*</label>
                <textArea id="memo" name="memo" class="form-control" >{{$check->memo}}</textArea>
                @if($errors->has('memo'))
                    <em class="invalid-feedback">
                        {{ $errors->first('memo') }}
                    </em>
