...
class DispenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        abort_unless(\Gate::allows('dispense_access'), 403);
        $checker = [];
        ...
        if (!in_array('absence_all_access', $checker)) {
            $qry = AbsenceRequest::selectRaw('absence_requests.*, staffs.name as staff_name')
                ...
                ->where(function ($query) {
                    $query->where('absence_requests.category', 'dispense')
                        ->orWhere('absence_requests.category', 'dispense_special');
                })
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('actions', function ($row) {
                $viewGate = 'dispense_access';
                //$editGate = 'dispense_edit';
                $editGate = '';
                $deleteGate = 'dispense_delete';
                $crudRoutePart = 'dispense';
        ...
        return view('admin.dispense.index');
    }
    public function create(Request $request)
    {
        abort_unless(\Gate::allows('dispense_create'), 403);
        $type = $request->type;
        $staffs = Staff::orderBy('name')->get();
        return view('admin.dispense.create', compact('staffs', 'type'));
    }
    public function store(Request $request)
    {
        abort_unless(\Gate::allows('dispense_create'), 403);
        $dispense = AbsenceRequest::create($request->all());
        return redirect()->route('admin.dispense.index');
    }

    public function show($id)
    {
        abort_unless(\Gate::allows('dispense_access'), 403);
        $dispense = AbsenceRequest::selectRaw('absence_requests.*, staffs.name as staff_name')
            ->join('staffs', 'staffs.id', '=', 'absence_requests.staff_id')
        ...
        // dd($dispense, $file);
        return view('admin.dispense.show', compact('dispense', 'file', 'visit_images'));
    }

    public function edit($id)
    {
        abort_unless(\Gate::allows('dispense_edit'), 403);
        $requests = AbsenceRequest::where('id', $id)->first();
        $type = $requests->type;
        $staffs = Staff::orderBy('name')->get();
        return view('admin.dispense.edit', compact('staffs', 'type', 'requests'));
    }
    public function update(Request $request, $id)
    {
        abort_unless(\Gate::allows('dispense_edit'), 403);
        $dispense = AbsenceRequest::where('id', $id)
    ...
    public function reject($id)
    {
        ...
        $message = "Dispensasi anda tanggal " . $d->start . " sampai dengan " . $d->end . " ditolak";
        MessageLog::create([
            'staff_id' => $d->staff_id,
            'memo' => $message,
            'type' => 'message',
    ...
    public function approve($id)
    {
        ...
        $d = AbsenceRequest::where('id', $id)->first();
        $message = "Dispensasi anda tanggal " . $d->start . " sampai dengan " . $d->end . " Disetujui";
        MessageLog::create([
            'staff_id' => $d->staff_id,
            'memo' => $message,
            'type' => 'message',
    ...
    // {
    //     abort_unless(\Gate::allows('dispense_delete'), 403);
