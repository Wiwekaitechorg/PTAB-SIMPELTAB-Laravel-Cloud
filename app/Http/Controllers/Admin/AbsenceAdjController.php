...
class AbsenceAdjController extends Controller
{
    use TraitModel;

    public function absenceRecapTest(Request $request)
    {
        ...
        foreach ($staffs as $staff) {
            ...
            //get if any adjustment on this periode
            if ($request->adj_inc) {
                ...
                $absenceadjs = AbsenceAdj::where('id', '=', -1)->first();
            }
            ...
            $data[] = [
                ...
                "Jumlah_Lembur"             => floor($report->jumlah_lembur) + ($absenceadjs ? $absenceadjs->Jumlah_Lembur : 0),
                ...
                "Jumlah_Masuk_Label"              => ($report->jumlah_masuk + $report->jumlah_dispen),
                "Jumlah_" . $kegiatan . "1_Label" => $report->jumlah_k1,
                "Jumlah_" . $kegiatan . "2_Label" => $report->jumlah_k2,
                "Jumlah_Dinas_Dalam_Label"        => $report->jumlah_dinasDalam,
                "Jumlah_Dinas_Luar_Label"         => $jumlah_dinasLuar,
                "Jumlah_Cuti_Label"               => $jumlah_cuti,
                "Jumlah_Lembur_Label"             => floor($report->jumlah_lembur),
                "Jumlah_Lembur4_Label"            => $report->jumlah_lemburlebih,
                "Jumlah_Permisi_Label"            => $report->jumlah_permisi,
                "Jumlah_Izin_Label"               => $izin->jumlah_izin,
                "Jumlah_Sakit_Label"              => $sakit->jumlah_sakit,
                "Jumlah_Dispen_Label"             => $report->jumlah_dispen,
                "Jumlah_Alfa_Label"               => ($alpha > 0 ? $alpha : 0),
                "Jumlah_Tidak_Masuk_Label"        => ($jumlah_cuti + $izin->jumlah_izin + $sakit->jumlah_sakit + $alpha),
                "Jumlah_Hari_Kerja_Label"         => $jumlah_kerja,
                "Jumlah_Hari_Libur_Label"         => $jumlah_libur,
                "Absen_Bolong_Datang_Label"       => $absen_bolong_datang,
                "Absen_Bolong_Pulang_Label"       => $absen_bolong_pulang, //
                "Absen_Lambat_Label"              => $report->jumlah_lambat,
                "Permisi_Potong_Gaji_Label"       => $permisi_potong_gaji,
                "Permisi_Tidak_Potong_Gaji_Label" => $permisi_tidak_potong_gaji,
            ];
        }
    ...
    public function absenceRecap(Request $request)
    {
        ini_set("memory_limit", -1);
        set_time_limit(0);
    ...
    {
        abort_unless(\Gate::allows('absenceadj_access'), 403);

        $absenceadjs = AbsenceAdj::with('user')->with('staff')->get();

        return view('admin.absenceadj.index', compact('absenceadjs'));
    }

    public function create()
    {
        abort_unless(\Gate::allows('absenceadj_create'), 403);
        $staffs = Staff::orderBy('name', 'asc')->get();

        return view('admin.absenceadj.create', compact('staffs'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $customRequest = new Request([
            ...
            'from'     => $request->session_start,
            'to'       => $request->session_end,
        ]);

        $absenceRecap        = $this->absenceRecap($customRequest);
        $absenceadjs_getdata = $absenceRecap->getData();
        $absenceadjs         = $absenceadjs_getdata->data[0];
        $data_create         = [
            'user_id'                   => auth()->id(),
            'staff_id'                  => $request->staff_id,
            ...
            'memo'                      => $request->memo,
            'Jumlah_Masuk'              => $request->Jumlah_Masuk > 0 ? ($request->Jumlah_Masuk - ($absenceadjs ? $absenceadjs->Jumlah_Masuk : 0)) : 0,
            'Jumlah_Kegiatan1'          => $request->Jumlah_Kegiatan1 > 0 ? ($request->Jumlah_Kegiatan1 - ($absenceadjs ? $absenceadjs->Jumlah_Kegiatan1 : 0)) : 0,
            'Jumlah_Kegiatan2'          => $request->Jumlah_Kegiatan2 > 0 ? ($request->Jumlah_Kegiatan2 - ($absenceadjs ? $absenceadjs->Jumlah_Kegiatan2 : 0)) : 0,
            'Jumlah_Dinas_Dalam'        => $request->Jumlah_Dinas_Dalam > 0 ? ($request->Jumlah_Dinas_Dalam - ($absenceadjs ? $absenceadjs->Jumlah_Dinas_Dalam : 0)) : 0,
            'Jumlah_Dinas_Luar'         => $request->Jumlah_Dinas_Luar > 0 ? ($request->Jumlah_Dinas_Luar - ($absenceadjs ? $absenceadjs->Jumlah_Dinas_Luar : 0)) : 0,
            'Jumlah_Cuti'               => $request->Jumlah_Cuti > 0 ? ($request->Jumlah_Cuti - ($absenceadjs ? $absenceadjs->Jumlah_Cuti : 0)) : 0,
            'Jumlah_Lembur'             => $request->Jumlah_Lembur > 0 ? ($request->Jumlah_Lembur - ($absenceadjs ? $absenceadjs->Jumlah_Lembur : 0)) : 0,
            'Jumlah_Lembur4'            => $request->Jumlah_Lembur4 > 0 ? ($request->Jumlah_Lembur4 - ($absenceadjs ? $absenceadjs->Jumlah_Lembur4 : 0)) : 0,
            'Jumlah_Permisi'            => $request->Jumlah_Permisi > 0 ? ($request->Jumlah_Permisi - ($absenceadjs ? $absenceadjs->Jumlah_Permisi : 0)) : 0,
            'Jumlah_Izin'               => $request->Jumlah_Izin > 0 ? ($request->Jumlah_Izin - ($absenceadjs ? $absenceadjs->Jumlah_Izin : 0)) : 0,
            'Jumlah_Sakit'              => $request->Jumlah_Sakit > 0 ? ($request->Jumlah_Sakit - ($absenceadjs ? $absenceadjs->Jumlah_Sakit : 0)) : 0,
            'Jumlah_Dispen'             => $request->Jumlah_Dispen > 0 ? ($request->Jumlah_Dispen - ($absenceadjs ? $absenceadjs->Jumlah_Dispen : 0)) : 0,
            'Jumlah_Alfa'               => $request->Jumlah_Alfa > 0 ? ($request->Jumlah_Alfa - ($absenceadjs ? $absenceadjs->Jumlah_Alfa : 0)) : 0,
            'Jumlah_Tidak_Masuk'        => $request->Jumlah_Tidak_Masuk > 0 ? ($request->Jumlah_Tidak_Masuk - ($absenceadjs ? $absenceadjs->Jumlah_Tidak_Masuk : 0)) : 0,
            'Jumlah_Hari_Kerja'         => $request->Jumlah_Hari_Kerja > 0 ? ($request->Jumlah_Hari_Kerja - ($absenceadjs ? $absenceadjs->Jumlah_Hari_Kerja : 0)) : 0,
            'Jumlah_Hari_Libur'         => $request->Jumlah_Hari_Libur > 0 ? ($request->Jumlah_Hari_Libur - ($absenceadjs ? $absenceadjs->Jumlah_Hari_Libur : 0)) : 0,
            'Absen_Bolong_Datang'       => $request->Absen_Bolong_Datang > 0 ? ($request->Absen_Bolong_Datang - ($absenceadjs ? $absenceadjs->Absen_Bolong_Datang : 0)) : 0,
            'Absen_Bolong_Pulang'       => $request->Absen_Bolong_Pulang > 0 ? ($request->Absen_Bolong_Pulang - ($absenceadjs ? $absenceadjs->Absen_Bolong_Pulang : 0)) : 0,
            'Absen_Lambat'              => $request->Absen_Lambat > 0 ? ($request->Absen_Lambat - ($absenceadjs ? $absenceadjs->Absen_Lambat : 0)) : 0,
            'Permisi_Potong_Gaji'       => $request->Permisi_Potong_Gaji > 0 ? ($request->Permisi_Potong_Gaji - ($absenceadjs ? $absenceadjs->Permisi_Potong_Gaji : 0)) : 0,
            'Permisi_Tidak_Potong_Gaji' => $request->Permisi_Tidak_Potong_Gaji > 0 ? ($request->Permisi_Tidak_Potong_Gaji - ($absenceadjs ? $absenceadjs->Permisi_Tidak_Potong_Gaji : 0)) : 0,
        ];
        //return $data_create;
        $absenceadjs_exist = AbsenceAdj::whereDate('session_start', '=', $request->session_start)->whereDate('session_end', '=', $request->session_end)->where('staff_id', '=', $request->staff_id)->first();
        if (! $absenceadjs_exist) {
            AbsenceAdj::create($data_create);
        } else {
            $absenceadjs_exist->update($data_create);
        }

        return redirect()->route('admin.absenceadj.index');
    }
    ...
    public function edit($id)
    {
        ...
        abort_unless(\Gate::allows('absenceadj_edit'), 403);

        $absenceadj = AbsenceAdj::findOrFail($id);
        $staffs     = Staff::orderBy('name', 'asc')->get();

        return view('admin.absenceadj.edit', compact('absenceadj', 'staffs'));
    }

    public function update(Request $request, AbsenceAdj $absenceadj)
    {
        abort_unless(\Gate::allows('absenceadj_edit'), 403);
        ...
        //return $absenceadj;
        $absenceadj->update($data_create);

        return redirect()->route('admin.absenceadj.index');
    }

    public function destroy(AbsenceAdj $absenceadj)
    {
        abort_unless(\Gate::allows('absenceadj_delete'), 403);
