...
class AbsenceController extends Controller
{
    ...
    public function reportAllExcel(Request $request)
    {
        ...
        foreach ($staffs as $staff) {
            ...
            if ($staff->work_type_id === 1) {
                ...
                $holiday =  Holiday::selectRaw('count(id) jumlah_libur')->whereRaw("DATE(start) >= '".$date_start."' AND DATE(start) <= '".$date_end."'")->first();
                $jumlah_libur =  $holiday->jumlah_libur;
                $jumlah_kerja = $jumlah_kerja - $jumlah_libur;
                $alpha = $jumlah_kerja - $report->jumlah_masuk;

                $tanpaK = $jumlah_kerja - $report->jumlah_masuk - $report->jumlah_dinasLuar - $report->jumlah_cuti - $report->jumlah_izin - $report->jumlah_dispen;
            } else {
            ...
            $data[] = [
                ...
                'tanpa keterangan' => $tanpaK >= 0 ? $tanpaK : "0"
                // 'tess' => $staff->work_type_id
            ];
        }

        return Excel::download(new AbsenceAllExport($data), 'report_absence_all.xlsx');
    ...
    public function absenceDelete(Request $request)
    {
        ...
        if ($request->from < $max_date && $request->to < $max_date) {
            ...
            Absence::whereRaw("DATE(absence_logs.start_date) >= '".$request->from."' AND DATE(absence_logs.start_date) <= '".$request->to."'")->delete();
            //AbsenceLog::whereBetween(DB::raw('DATE(absence_logs.register)'), [$request->from, $request->to])->delete();
            Absence::whereRaw("DATE(absence_logs.register) >= '".$request->from."' AND DATE(absence_logs.register) <= '".$request->to."'")->delete();
            //AbsenceRequest::whereBetween(DB::raw('DATE(absence_requests.end)'), [$request->from, $request->to])->delete();
            Absence::whereRaw("DATE(absence_requests.end) >= '".$request->from."' AND DATE(absence_requests.end) <= '".$request->to."'")->delete();
            return redirect()->route('admin.absence.index');
        } else {
