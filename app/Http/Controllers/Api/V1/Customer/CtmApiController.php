...
class CtmApiController extends Controller
{
    ...
    public function ctmPay($id)
    {
        try {
            ...
            // $ctm_num_row = count($ctm) - 1;
            // $ctm_num_row = count($ctm) - $ctm_lock;
            ...
            foreach ($ctm as $key => $item) {
                ...
                //if status 0
                if ($date_now < $date_comp) {
                    if ($ctm[$key]->status == 0 && $ctm[$key]->bulanrekening == date('n', strtotime('-1 month')) && $ctm[$key]->tahunrekening == date('Y', strtotime('-1 month'))) {
                        unset($ctm[$key]);
                    }
                } else {
                }
    ...
    public function ctmRequest(Request $request)
    {
        ...
        CURLOPT_URL => 'https://simpletabadmin.ptab-vps-storage.com/api/open/customer/virmach-image-store',
        ...
        //$img_name_noext = $var['norek'] . "_" . $year_catat . "_" . $month_catat;
        //Storage::disk('sftp')->put($path_img_name, $request->file('image'));
        $path = Storage::disk('sftp')->putFileAs($path, $request->file('image'), $new_image_name);
