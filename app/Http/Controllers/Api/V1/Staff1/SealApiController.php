...
class SealApiController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $cekInput = LockAction::where('customer_id', $id)->where('created_at', 'like', date('Y-m') . '%')->where('type', $tindakan)->get();
        if (count($cekInput) >= 1) {
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 1; $i <= $dataQtyImage; $i++) {
            if ($request->file('image' . $i)) {
                ...
                $nameImage = $request->file('image' . $i)->getClientOriginalName();
