...
class RequestApiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // pengecekan lebur dan dinas hanya di jam kerja start
        else if ($dataForm->category == "leave" || $dataForm->category == "permission" || $dataForm->category == "dispense") {
            ...
            $absen_check = Absence::where('staff_id', $dataForm->staff_id)
                ...
                ->whereRaw("DATE(absences.created_at) >= '".$start."' AND DATE(absences.created_at) <= '".$end."'")
                //->where('register', '!=', null)
            ...
            $cek = AbsenceRequest::where('staff_id', $dataForm->staff_id)
                ...
                ->first();
            // dd($cek);
            $cek_sick = AbsenceRequest::where('staff_id', $dataForm->staff_id)
                ->where(function ($query) use ($start, $end) {
            ...
            if ($cek) {
                ...
                $error = "Anda Masih Memiliki Cuti/Izin sakit yang masih aktif";
            }
        ...
        } else if ($dataForm->category == "dispense_special") {
            //check if dispense_special already on this date
            $cek = AbsenceRequest::where('staff_id', $dataForm->staff_id)
            ...
            if ($cek) {
                $error = "Anda Sudah Memiliki Pengajuan Dispensasi Khusus di tanggal ini";
            }
            //set $startS
            $startS = date("Y-m-d H:i:s", strtotime($dataForm->start . $dataForm->time_start));            
            //set $endS
            $endS = date("Y-m-d H:i:s", strtotime($dataForm->end . $dataForm->time_end));
        } else {
        }
        ...
        if ($error == "") {
            ...
            // nanti diaktifkan
            if ($dataForm->category == "excuse" && $dataForm->type == "back") {
                $requests->attendance = $dataForm->attendance ? $dataForm->attendance : null;
            }
            ...
            //send notif to personalia
            $admin_arr = User::where('email', 'ekasuarmita@ptab-vps.com')->orWhere('email', 'indrabayu-personalia@ptab-vps.com')->orWhere('email', 'wedhanaadi@ptab-vps.com')->get();
            foreach ($admin_arr as $key => $admin) {
                $id_onesignal = $admin->_id_onesignal;
                // $message = 'Admin: Keluhan Baru Diterima : ' . $dataForm->description;
                //wa notif                
    ...
    public function getPermissionCatNew(Request $request)
    {
        $cat = [
            ['id' => 'sick', 'name' => 'sakit', 'checked' => false],
            ['id' => 'sick_proof', 'name' => 'sakit izin dokter', 'checked' => false],
            ['id' => 'other', 'name' => 'Lain-Lain', 'checked' => false],
        ];
        return response()->json([
            'message' => 'Pengajuan Terkirim',
