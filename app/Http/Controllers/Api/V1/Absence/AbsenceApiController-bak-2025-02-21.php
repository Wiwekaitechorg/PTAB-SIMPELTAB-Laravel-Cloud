        ...
        foreach ($staffs as $stf) {
            ...
            foreach ($absence as $data) {
                ...
                if ($clock_in != '') {
                    ...
                    //get late
                    $flag = ($get_clock_in->late > 0.016667 && $status == 'masuk') ? 'lambat' : '';
                ...
                $list_absen[] = [
                    ...
                    'Clock_in'   => $clock_in ? date('H:i', strtotime($clock_in)) : 'null',
                    'Clock_out'  => $clock_out ? date('H:i', strtotime($clock_out)) : 'null',
                    // 'Normal'     => '',
                    // 'Real time'  => '',
                    // 'Late'       => $shift,
                    // 'Early'      => '',
            ...
            if ($staff->work_type_id === 2) {
                ...
                foreach ($dates as $dt) {
                    ...
                    if ($list != null) {
                        $cek_masuk = $list_absen->where('Date', $dt->format('Y-m-d'))->where('status', 'masuk')->first();
    ...
    public function reportAllExcel(Request $request)
    {
        ...
        foreach ($staffs as $staff) {
            ...
            $data[] = [
                ...
                "Jumlah_Masuk"              => ($report->jumlah_masuk + $report->jumlah_dispen),
                "Jumlah_" . $kegiatan . "1" => $report->jumlah_k1,
                "Jumlah_" . $kegiatan . "2" => $report->jumlah_k2,
                "Jumlah_Dinas_Dalam"        => $report->jumlah_dinasDalam,
                "Jumlah_Dinas_Luar"         => $jumlah_dinasLuar,
                "Jumlah_Cuti"               => $report->jumlah_cuti,
                "Jumlah_Lembur"             => $report->jumlah_lembur,
                "Jumlah_Lembur4"            => $report->jumlah_lemburlebih,
                ...
                "Jumlah_Alfa"               => $alpha > 0 ? $alpha : 0,
                "Jumlah_Tidak_Masuk"        => ($report->jumlah_cuti + $izin->jumlah_izin + $sakit->jumlah_sakit + $alpha),
                "Jumlah_Hari_Kerja"         => $jumlah_kerja,
                "Jumlah_Hari_Libur"         => $jumlah_libur,
                "Absen_Bolong_Datang"        => $absen_bolong_datang,
                "Absen_Bolong_Pulang"       => $absen_bolong_pulang, //
                "Absen_Lambat"              => $report->jumlah_lambat,
                "Permisi_Potong_Gaji"       => $permisi_potong_gaji,
                "Permisi_Tidak_Potong_Gaji" => $permisi_tidak_potong_gaji,
            ];
        }
