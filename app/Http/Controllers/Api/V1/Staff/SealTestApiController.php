...
class SealTestApiController extends Controller
{
    use TraitModel;
        ...
        if ($user->staff_id != 0 || $user->staff_id != null) {
            ...
            if ($date_now > $date_comp) {
                if ($request->search != '') {
                    ...
                    $qry = $qry->groupBy('tblpembayaran.nomorrekening')->get();
                } else {
                    $tes = "2";
                    $data = AreaStaff::select('area_id')->where('staff_id', $user->staff_id)->get();
                    if (count($data) > 0) {
    ...
    public function history(Request $request, $id)
    {
        ...
        if ($request->status != '' && $request->search != ''  && $request->date != '') {
            ...
            $qry = LockAction::join('simpletabctm.tblpelanggan', 'lock_action.customer_id', '=', 'tblpelanggan.nomorrekening')->where('staff_id', $id)->get();
        }
